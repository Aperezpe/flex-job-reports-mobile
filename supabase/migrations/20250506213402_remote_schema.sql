CREATE TRIGGER on_auth_user_created AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION private.handle_user_registration();
...
create policy "Allow users of their company to handle images 1j2qn9y_0"
on "storage"."objects"
as permissive
for select
to authenticated
using (((bucket_id = 'job-reports-images'::text) AND (EXISTS ( SELECT 1
   FROM users
  WHERE (users.id = auth.uid()))) AND ((split_part(name, '/'::text, 1))::uuid = ( SELECT users.company_id
   FROM users
  WHERE (users.id = auth.uid())))));


create policy "Allow users of their company to handle images 1j2qn9y_1"
on "storage"."objects"
as permissive
for insert
to authenticated
with check (((bucket_id = 'job-reports-images'::text) AND (EXISTS ( SELECT 1
   FROM users
  WHERE (users.id = auth.uid()))) AND ((split_part(name, '/'::text, 1))::uuid = ( SELECT users.company_id
   FROM users
  WHERE (users.id = auth.uid())))));
...
create policy "Allow users of their company to handle images 1j2qn9y_2"
on "storage"."objects"
as permissive
for delete
to authenticated
